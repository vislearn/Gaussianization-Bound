...
# From https://github.com/biweidai/SINF/blob/master/sinf/SlicedWasserstein.py
        ...
        with torch.no_grad():
            ...
            while loss1 > loss + c * m * lr:
                ...
                if 2 * K < ndim:
                    ...
                    AT1 = (AT.double() - lr * AT.double() @ V @ torch.pinverse(
                        torch.eye(2 * K, dtype=torch.double, device=x.device) + lr / 2 * UT @ V) @ UT).to(
                        torch.get_default_dtype())
                else:
                    AT1 = (AT.double() @ (torch.eye(ndim, dtype=torch.double,
                                                    device=x.device) - lr / 2 * BT.double()) @ torch.pinverse(
                        torch.eye(ndim, dtype=torch.double, device=x.device) + lr / 2 * BT.double())).to(
                        torch.get_default_dtype())
    ...
    if second != 'gaussian':
        ...
        if batchsize is None:
            ...
            while i * batchsize < data0.shape[1]:
                SWD[i * batchsize: (i + 1) * batchsize] = Objective(data0[:, i * batchsize: (i + 1) * batchsize].T,
                                                                    second0[:, i * batchsize: (i + 1) * batchsize].T, p,
                                                                    perdim=False)
                i += 1
    else:
        ...
        if batchsize is None:
            ...
            i = 0
            while i * batchsize < data0.shape[1]:
                SWD[i * batchsize: (i + 1) * batchsize] = ObjectiveG(data0[:, i * batchsize: (i + 1) * batchsize].T, pg,
                                                                     p, w=weight, perdim=False)
                i += 1
            ...
            for p in group['params']:
                ...
                if p.data.ndim == 2:
                    ...
                    if 2 * K < n_dim:
                        ...
                        p.data = (X - group['lr'] * U @ torch.pinverse(
                            torch.eye(2 * K, dtype=torch.double, device=X.device) + group[
                                'lr'] / 2. * VT @ U) @ VT @ X).type(dtype)
                    else:
                        A = G @ X.T - X @ G.T
                        p.data = (torch.pinverse(
                            torch.eye(n_dim, dtype=torch.double, device=X.device) + group['lr'] / 2 * A) @ (
                                              torch.eye(n_dim, dtype=torch.double, device=X.device) - group[
                                          'lr'] / 2 * A) @ X).type(dtype)
                elif p.data.ndim == 3:
                    ...
                    if 2 * K < n_dim:
                        for i in range(len(p.data)):
                            ...
                            p.data[i] = (X[i] - group['lr'] * U @ torch.pinverse(
                                torch.eye(2 * K, dtype=torch.double, device=X.device) + group[
                                    'lr'] / 2. * VT @ U) @ VT @ X[i]).type(dtype)
                    else:
                        for i in range(len(p.data)):
                            A = G[i] @ X[i].T - X[i] @ G[i].T
                            p.data[i] = (torch.pinverse(
                                torch.eye(n_dim, dtype=torch.double, device=X.device) + group['lr'] / 2 * A) @ (
                                                     torch.eye(n_dim, dtype=torch.double, device=X.device) - group[
                                                 'lr'] / 2 * A) @ X[i]).type(dtype)
                else:
